...
class ScreenSpeak:
    ...
    def _save_and_play_audio(self, audio_content, identifier):
        ...
        audio_file_path = os.path.join(self.audio_transcripts_dir, audio_filename)  # Adjusted path

        with open(audio_file_path, 'wb') as audio_file:
            audio_file.write(audio_content)

        print(f"Saved audio to {audio_file_path}")
        subprocess.run(['mpg123', audio_file_path], check=True)  # Adjusted to use full path
    ...
    def _process_screenshot(self, screenshot_path):
        ...
        try:
            ...
            description_openai_response = self._generate_script(screenshot_path)
            description_openai = description_openai_response.content if hasattr(description_openai_response, 'content') else str(description_openai_response)
            print("OpenAI-generated script:", description_openai)
    
            # Generate the script using Anthropic
            description_anthropic_response = self._generate_script_anthropic(screenshot_path)
            description_anthropic = description_anthropic_response.content if hasattr(description_anthropic_response, 'content') else str(description_anthropic_response)
            print("Anthropic-generated script:", description_anthropic)
    
            # Generate a common AI filename base for all associated files
            file_name_base = self._generate_file_name(description_openai + " " + description_anthropic)
    
            # Save individual answers with prepended LLM names
            self._save_individual_answer(description_openai, "ChatGPT_" + file_name_base)
            self._save_individual_answer(description_anthropic, "Anthropic_" + file_name_base)
    
            # Synthesize the best answer from both models' outputs
            synthesized_description_response = self._synthesize_best_answer(description_openai, description_anthropic)
            synthesized_description = synthesized_description_response.content if hasattr(synthesized_description_response, 'content') else str(synthesized_description_response)
            ...
            self._save_text_analysis(synthesized_description, file_name_base)
    ...
    def _save_individual_answer(self, answer, filename_base):
        """
        Saves an individual model's answer to a text file.
    
        :param answer: The answer generated by the model.
        :param filename_base: The base name for the file, with prepended model name.
        """
        filename = f"{filename_base}.txt"
        filepath = os.path.join(self.text_analysis_output_dir, filename)
        with open(filepath, 'w', encoding='utf-8') as file:
            file.write(answer)
        print(f"Saved {filename_base} answer to {filepath}")
